void applyPrizmaEffect(vec2 uv, vec4 baseColor, vec4 effectColor, vec4 effectParams,
    ...
    float rows = max(effectParams.y, 3.0);
    float row = floor(vNorm * rows);
    float t = gameTime * speed;
    float hueT = fract(row * 0.11 + t * 0.15);
    float r = clamp(abs(hueT * 6.0 - 3.0) - 1.0, 0.0, 1.0);
    float g = clamp(2.0 - abs(hueT * 6.0 - 2.0), 0.0, 1.0);
    float b = clamp(2.0 - abs(hueT * 6.0 - 4.0), 0.0, 1.0);
    vec3 color = mix(baseColor.rgb, vec3(r, g, b), 0.8);
