void applyWarpEffect(vec2 uv, vec4 baseColor, vec4 effectColor, vec4 effectParams,
    ...
    float rad = length(p);
    float sw = sin(ang * 3.0 - t * 2.0 + rad * 5.0);

    vec2 uvO = uv + vec2(cos(ang + t * 0.7), sin(ang + t * 0.7)) * sw * 0.025 * strength;
    float aO = texture(tex, uvO).a;

    vec3 color = mix(baseColor.rgb, effectColor.rgb, 0.3 + 0.35 * sw);
