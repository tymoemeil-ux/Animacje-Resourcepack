void applyCukierekEffect(vec2 uv, vec4 baseColor, vec4 effectColor, vec4 effectParams,
    ...
    float width = max(effectParams.y, 1.0);
    float t = gameTime * speed;
    float s = fract((uNorm * 0.6 + (1.0 - vNorm) * 0.4) * width - t * 0.5);
    float stripe = step(0.5, s);
    vec3 color = mix(effectColor.rgb, vec3(1.0), stripe) * 0.85 + 0.12;
