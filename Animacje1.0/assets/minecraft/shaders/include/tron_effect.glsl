void applyTronEffect(vec2 uv, vec4 baseColor, vec4 effectColor, vec4 effectParams,
    ...
    float grid = max(effectParams.y, 3.0);
    float t = gameTime * speed;
    vec2 g = vec2(uNorm, 1.0 - vNorm) * grid;
    vec2 f = abs(fract(g + vec2(0.0, t * 0.3)) - 0.5);
    float line = 1.0 - smoothstep(0.0, 0.08, min(f.x, f.y));
    vec3 color = baseColor.rgb * 0.35 + effectColor.rgb * line * 1.45;
