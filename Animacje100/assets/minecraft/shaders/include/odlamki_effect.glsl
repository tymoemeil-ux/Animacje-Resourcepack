void applyOdlamkiEffect(vec2 uv, vec4 baseColor, vec4 effectColor, vec4 effectParams,
    ...
    float shards = max(effectParams.y, 2.0);
    vec2 g = vec2(uNorm * 0.7 + (1.0 - vNorm) * 0.3, 1.0 - vNorm) * shards;
    vec2 cell = floor(g);
    vec2 f = fract(g);
