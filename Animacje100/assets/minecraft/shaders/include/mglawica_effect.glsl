void applyMglawicaEffect(vec2 uv, vec4 baseColor, vec4 effectColor, vec4 effectParams,
    ...
    float n1 = 0.5 + 0.5 * sin((uNorm * 4.0 + t * 0.3 * drift) * 1.5);
    float n2 = 0.5 + 0.5 * sin((vNorm * 5.0 - t * 0.2 * drift) * 1.7 + 2.0);
    float n = n1 * n2;
    vec3 c1 = vec3(0.35, 0.15, 0.5);
    ...
    float star = step(0.93, h2) * (0.5 + 0.5 * sin(t * 2.0 + h2 * 25.0)) * (1.0 - smoothstep(0.0, 0.35, length(f - 0.5) * 2.0));
    color += vec3(1.0) * star * 0.9;
