void applyIskreniceEffect(vec2 uv, vec4 baseColor, vec4 effectColor, vec4 effectParams,
    ...
    vec2 cell = floor(g);
    vec2 f = fract(g);
    float tStep = floor(gameTime * speed * 6.0);
    float h2 = fract(sin(dot(vec2(cell.x * 23.17 + tStep, cell.y * 7.0 + tStep), vec2(12.9898, 78.233))) * 43758.5453);
    float on = step(0.55, h2);
    float tw = sin(fract(h2 * 17.0 + gameTime * speed * 2.0) * 3.14159 * 2.0) * 0.5 + 0.5;
    float star = (1.0 - smoothstep(0.0, 0.35, length(f - 0.5) * 2.0)) * (0.5 + 0.5 * tw);
    vec3 gold = mix(vec3(1.0, 0.85, 0.4), vec3(1.0, 1.0, 0.8), h2);
    vec3 color = baseColor.rgb + gold * star * on * 1.6;
